...
public class UI_Tools : MonoSingleton<UI_Tools>
{
	...
	BaseObject GetUI(eUIType uIType, bool isDontDestroy = false)
	{
		...
		GameObject prefabUI = Resources.Load("Prefabs/UI/" +
			uIType.ToString()) as GameObject;
		...
		if (prefabUI != null)
		{
			if (isDontDestroy == false)
			{
				...
				if (baseObject == null)
				{
					Debug.Log(uIType.ToString() + " 오브젝트에 " +
						" BaseObject가 연결되어 있지 않습니다.");
	...
	public void SubRootCreate()
	{
		...
		SubUIRoot.transform.position =
			uiRoot.transform.position;
		SubUIRoot.transform.localScale =
