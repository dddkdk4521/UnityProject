...
public class BoardManager : MonoSingleton<BoardManager>
{
    ...
    Dictionary<eBoardType, GameObject> DicPrefab = 
                 new Dictionary<eBoardType, GameObject>();

    private void Awake()
    {
        if (this.BoardUI == null)
        {
            this.BoardUI = new GameObject("BoardUI_Root");
            this.BoardUI.layer = LayerMask.NameToLayer("UI");

            this.BoardUI.transform.SetParent(UIManager.Instance.transform);
            this.BoardUI.transform.localPosition = Vector3.zero;
            this.BoardUI.transform.localScale = Vector3.one;
        }
    ...
    BaseBoard MakeBoard(eBoardType boardType)
    {
        ...
        switch (boardType)
        {
            ...
            case eBoardType.BOARD_HP:
                {
                    GameObject hpBoard = 
                        NGUITools.AddChild(BoardUI, DicPrefab[eBoardType.BOARD_HP]);
                    boardData = hpBoard.GetComponent<HpBoard>();
                }
                break;
            case eBoardType.BOARD_DAMAGE:
                {
                    GameObject damageBoard = 
                        NGUITools.AddChild(BoardUI, DicPrefab[eBoardType.BOARD_DAMAGE]);
    ...
    public BaseBoard GetBoadData(BaseObject keyObject, eBoardType boardType)
    {
        if (DicBoard.ContainsKey(keyObject) == false)
        {
            return null;
    ...
    public void ClearBoard(BaseObject keyObject, bool bEnable = true)
    {
        if (DicBoard.ContainsKey(keyObject) == false)
        {
            return;
